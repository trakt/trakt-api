...
// FIXME: remove when https://github.com/ts-rest/ts-rest/issues/802 is closed
...
const getPathParameters = (path: string, zodObject?: unknown) => {
  ...
  const params: any[] = paramsFromPath?.map((param) => ({
    name: param,
    in: 'path' as const,
    required: true,
    schema: {
...
export const generateOpenApi = (
  ...
  const pathObject = paths.reduce((acc, path) => {
    ...
    let bodySchema = path.route?.method !== 'GET' && 'body' in path.route
