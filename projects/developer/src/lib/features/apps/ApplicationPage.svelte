  ...
  const title = $derived(
    mode === "list"
      ...
      : mode === "new"
        ? "Create an app"
        : mode === "edit"
          ? `Edit ${appName ?? (session.loading ? "Loading…" : "app")}`
          : (appName ?? "App details"),
  );
</script>
...
{:else}
  <section class="welcome">
    <nav aria-label="Breadcrumb">
      ...
      >
      {#if mode === "edit"}
        / <a href={applicationUrl(appId, appName)}
          >{appName ?? (session.loading ? "Loading…" : "App details")}</a
        >
      {/if}
      {#if mode !== "list"}
        / <span aria-current="page"
          >{mode === "new"
            ...
            : mode === "edit"
              ? "Edit"
              : (appName ??
                (session.loading ? "Loading…" : "App details"))}</span
        >
      {/if}
    </nav>
    <h1>{title}</h1>
